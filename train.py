from dataset.dataset_org import Im2LatexDataset
import os
import argparse
import logging
import yaml
...
from utils import in_model_path, parse_args, seed_everything, get_optimizer, get_scheduler, gpu_memory_check
...
def train(args):
    ...
    # print(len(next(iter(dataloader))[0]['input_ids']))
    # print("--"*30)
    ...
    def save_models(e, step=0, bleu=00):
        torch.save(model.state_dict(), os.path.join(out_path, '%s_e%02d_step%02d_b%.02f.pth' % (args.name, e+1, step, bleu)))
        yaml.dump(dict(args), open(os.path.join(out_path, 'config_e%02d.yaml' % (e+1)), 'w+'))
            ...
            for i, (seq, im) in enumerate(dset):
                ...
                if (i+1+len(dataloader)*e) % args.sample_freq == 0:
                    ...
                    if bleu_score > max_bleu and token_accuracy > max_token_acc:
                        ...
                        save_models(e, step=i, bleu=bleu_score)
