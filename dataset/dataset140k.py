...
class Im2LatexDataset:
        ...
        if pth is not None:
            ...
            try:
                ...
                for i in tqdm(self.df.index):
                    width, height = self.df['width'][i], self.df['height'][i]
                    if min_dimensions[0] <= width <= max_dimensions[0] and min_dimensions[1] <= height <= max_dimensions[1]:
                        ...
                        self.data[(width, height)].append((self.df['latex_ascii'][i], image_path))
    ...
    def prepare_data(self, batch):
        ...
        for path in list(ims):
            path = os.path.realpath(os.path.join('dataset\\data', path.split('/')[-2], path.split('/')[-1]))
            print("#"*50)
            print(path)
            print("#"*50)
            im = cv2.imread(path)
...
if __name__ == '__main__':
    ...
    if args.gen_dataset == "True":
        ...
        # for type in types:
        pth = os.path.realpath(os.path.join('dataset/data/','df_{}.pkl'.format(args.type)))
        if dataset is None:
