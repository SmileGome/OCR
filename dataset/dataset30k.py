# generate dataset for 30k
# train 23701 / valid 2932 / test 2932
# save as train30k.pkl / valid30k.pkl / test30k.pkl
...
from utils.utils import in_model_path, pad
...
class Im2LatexDataset:
    ...
    def __init__(self, pth=None, tokenizer=None, shuffle=True, batchsize=16, max_seq_len=1024,
        ...
        if pth is not None:
            ...
            self.df = pd.read_pickle(os.path.realpath(os.path.join('dataset/data/', pth)))
            # self.images = [path.replace('\\', '/') for path in glob.glob(join(images, '*.png'))] # 이미지 경로들 리스트
            # self.images = [os.path.realpath(os.path.join(args.images, self.df['image'][i], '.png')) for i in self.df.index]
            self.sample_size = self.df.shape[0]
            # self.sample_size = len(self.images) # 이미지 개수
            # eqs = open(equations, 'r').read().split('\n')   # 레이텍스 읽기
            # self.indices = [int(os.path.basename(img).split('.')[0]) for img in self.images]    # 이미지 이름에서 index?만 추출한 리스트
            self.tokenizer = PreTrainedTokenizerFast(tokenizer_file=tokenizer)  # 토크나이저 부르기
            ...
            try:
                # for i, im in tqdm(enumerate(self.df), total=self.df.shape[0]):
                for i in tqdm(self.df.index):
                    img = os.path.realpath(os.path.join(args.path, 'images\\', self.df['image'][i]+'.png'))
                    width, height = imagesize.get(img)
                    if min_dimensions[0] <= width <= max_dimensions[0] and min_dimensions[1] <= height <= max_dimensions[1]:
                        # image_path = os.path.realpath(os.path.join('dataset/data/formula_images/', self.df['image'][i]))  # MacOS
                        # image_path = os.path.realpath(os.path.join('dataset\\data\\formula_images\\', self.df['image'][i]))  # Windows  #140k

                        self.data[(width, height)].append((self.df['formula'][i], img))
    ...
    def prepare_data(self, batch):
        ...
        for path in list(ims):
            # path = os.path.realpath(os.path.join('dataset\\data', path.split('/')[-2], path.split('/')[-1]))    # 현재 140k 저장 형태 기준
            # print("#"*50)
            # print(path)
            # print("#"*50)
            im = cv2.imread(path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_dataset', default="False", type=str)
    ...
    if args.gen_dataset == "True":
        ...
        # for type in types:
        if args.type == None:
            raise Exception('type(train/valid/test)을 입력해주세요.')

        if args.path == None:
            raise Exception('dataset의 경로를 입력해주세요.')
    
        pth = os.path.realpath(os.path.join(args.path,'{}.pkl'.format(args.type)))
        if dataset is None:
            dataset = Im2LatexDataset(pth, args.tokenizer)
        else:
            dataset.combine(Im2LatexDataset(pth, args.tokenizer))
